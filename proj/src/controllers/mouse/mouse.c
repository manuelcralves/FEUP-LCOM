...
void mouse_update(struct packet * pacote) {
  ...
  if (pacote->delta_x > 0) {
    if (cursor->x + pacote->delta_x > (int)xRes - cursor->img.width)
      cursor->x = (int)xRes - cursor->img.width;
    else
  ...
  if (pacote->delta_y < 0) {
    if (cursor->y + cursor->img.height - pacote->delta_y > (int)yRes)
      cursor->y = (int)yRes - cursor->img.height;
    else
  ...
  for(int i = 0; i < cursor->img.width; i++) {
        for (int j = 0; j < cursor->img.height; j++) {
          if (*(map + i + j*cursor->img.width) != xpm_transparency_color(XPM_8_8_8_8))
            vg_draw_pixel(cursor->x+i,cursor->y+j,*(map + i + j*cursor->img.width));
    }
  }
}
...
unsigned int check_collision_main_menu () {
  if (cursor->x > 38 && cursor->x < 238 && cursor->y > 384 && cursor->y < 442)
    return 2;
  else if (cursor->x > 306 && cursor->x < 506 && cursor->y > 384 && cursor->y < 442)  
    return 3;
  else if (cursor->x > 574 && cursor->x < 774 && cursor->y > 384 && cursor->y < 442)
    return 1;
  else return 0;
}

unsigned int check_collision_pause_menu () {
  if (cursor->x > 150 && cursor->x < 350 && cursor->y > 384 && cursor->y < 442)
    return 1;
  else if (cursor->x > 450 && cursor->x < 650 && cursor->y > 384 && cursor->y < 442)  
    return 2;
  else return 0;
}

unsigned int checkCollisionWonMenu() {
  if (cursor->x > 320 && cursor->x < 520 && cursor->y > 484 && cursor->y < 542)
    return 1;
  else return 0;
}

unsigned int checkCollisionLostMenu() {
  if (cursor->x > 320 && cursor->x < 520 && cursor->y > 484 && cursor->y < 542)
    return 1;
  else return 0;
}
