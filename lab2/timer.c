...
int(timer_get_conf)(uint8_t timer, uint8_t *st) {
  uint8_t x = (TIMER_RB_CMD | TIMER_RB_COUNT_ | TIMER_RB_SEL(timer)); // Read Back Command
...
int(timer_display_conf)(uint8_t timer, uint8_t st, enum timer_status_field field) {
  ...
  if (field == tsf_all) {
    ...
    if ((st & TIMER_LSB) == 0)
      if ((st & TIMER_MSB) == 0)
        tsf_union.in_mode = INVAL_val;
      else
        tsf_union.in_mode = MSB_only;
    else if ((st & TIMER_MSB) == 0)
      tsf_union.in_mode = LSB_only;
    else
      tsf_union.in_mode = MSB_after_LSB;
  }
  else if (field == tsf_mode) {
    tsf_union.count_mode = st;

    uint8_t x = st & (TIMER_SQR_WAVE | BIT(3)); //     00001110

    switch (x) {
      case 0:
        tsf_union.count_mode = 0;
        break;
      case 2:
        tsf_union.count_mode = 1;
        break;
      case 4:
        tsf_union.count_mode = 2;
        break;
      case 6:
        tsf_union.count_mode = 3;
        break;
      case 8:
        tsf_union.count_mode = 4;
        break;
      case 10:
        tsf_union.count_mode = 5;
        break;
      case 12:
        tsf_union.count_mode = 2;
        break;
  ...
  if (timer_print_config(timer, field, tsf_union) != 0) {
    return 1;
  }
  return 0;
}
